...
export default function Login({ onNavigate, onLogin }) {
  ...
  const [ownerForm, setOwnerForm] = useState({ pgName: '', password: '' });
  ...
  const handleOwnerSubmit = async (e) => {
    e.preventDefault();
    if (!ownerForm.pgName.trim()) {
      setToast('❌ PG Name is required.');
      return;
    }
    ...
    setToast('');
    try {
      const data = await loginOwner({ pgName: ownerForm.pgName, password: ownerForm.password });
      ...
      <div className="login-card">
        {isOwnerLogin ? (
          ...
          <div>
            ...
            <p className="login-subtitle">Manage your PG listing, room availability, and bookings</p>

            <form onSubmit={handleOwnerSubmit}>
              <div className="form-group">
                <label className="form-label" htmlFor="owner-pgname">PG Name *</label>
                <input
                  id="owner-pgname"
                  className="form-input"
                  type="text"
                  placeholder="e.g. Comfort Zone PG"
              ...
              <div style={{ textAlign: 'right', marginTop: '-10px', marginBottom: '20px' }}>
                <span
                  ...
                  onClick={() => setToast('ℹ️ Contact PGFinder Support (support@pgfinder.com) to recover owner account.')}
                >
                  Forgot password?
                </span>
              </div>
            ...
            <button
              ...
              style={{ width: '100%', marginTop: '16px', border: '1px solid var(--border)' }}
              onClick={() => {
                setIsOwnerLogin(false);
                setOwnerForm({ pgName: '', password: '' });
                setToast('');
              }}
            >
              ← Back to Main Login
          ...
          <div>
            ...
            <div style={{ margin: '20px 0 10px', height: '1px', background: 'var(--border)' }} />
            
            <button 
              className="btn btn-ghost" 
              style={{ width: '100%', border: '1px solid var(--accent)', color: 'var(--accent)', fontWeight: '600' }}
